...
The data is then processed and analyzed with spark using the PySpark and PyOWM, DateUtil and Spark MLlib amongst others (see Data Processing). For the visualizations, we used Pandas in combination with matplotlib due to the familiarity of the group with the technology and because the size of the resulting dataset allowed us to step away from Big Data technologies. If a more successful result (see Data Analytics) was achieved, there would be other technologies optimal for the visualizations part.
...
For the data processing, our goal was to merge the two datasets in a way that would allow us to do our models and analysis without issues. As the joined dataset consists of almost 20,000 observations, we had to do this in a scalable matter and to be aware of the computing power necessary to perform it making the Spark environment ideal for this data project.
...
Linear regression is a modelling approach used to investigate the relationship between a scalar response variable and explanatory variables. In our case, the response variables are the subjec- tivity and polarity of the Tweets, and the explanatory variables are the weather data, more specifically we choose to investigate the parameters of dew point, cloud coverage, the subjective temperature, the humidity, the air pressure, the temperature, the visibility, the wind degrees, and the wind speed. The variables names and types can be seen in Figure 7 below.
